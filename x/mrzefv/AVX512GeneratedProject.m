...
static NSString *AVX512ObjCString(NSString *value)
{
    ...
    NSMutableString *result =
        [NSMutableString stringWithString:value];
...
static void AVX512AppendLine(NSMutableString *string,
                             NSString *line)
{
...
static NSString *AVX512GeneratedHeader(NSString *sessionID)
{
    ...
    AVX512AppendLine(
        output,
        @"FOUNDATION_EXPORT NSString *MRzefvGeneratedSessionID(void);");
    AVX512AppendLine(
        output,
                                       ...
                                       NSArray *operations)
{
    ...
    AVX512AppendLine(
        output,
        @"NSString *MRzefvGeneratedSessionID(void)");
    AVX512AppendLine(output, @"{");
    AVX512AppendLine(
        output,
        [NSString stringWithFormat:
    ...
    AVX512AppendLine(
        output,
        @"        UIView *found = MRzefvFindViewInTree(subview, targetClass);");
    AVX512AppendLine(output, @"");
    AVX512AppendLine(output,
                     @"        if (found) {");
    AVX512AppendLine(output,
    ...
    AVX512AppendLine(
        output,
        @"static void MRzefvApplyOperations(UIView *target)");
    AVX512AppendLine(output, @"{");
    for (id rawOperation in operations) {
        ...
        switch (type) {
            ...
            case AVX512GeneratedOperationTypeResize: {
                NSString *width =
                    [values[@"width"] description];
                NSString *height =
                    [values[@"height"] description];
                if (width && height) {
                    AVX512AppendLine(
                        output,
                        [NSString stringWithFormat:
            ...
            case AVX512GeneratedOperationTypeAddText: {
                ...
                if (text) {
                    AVX512AppendLine(
                        output,
                        @"    UILabel *label = [[UILabel alloc] initWithFrame:target.bounds];");
                    AVX512AppendLine(
                        output,
                                   ...
                                   error:(NSError **)error
{
    if (!targetView) {
        if (error) {
            *error =
                AVX512Error(@"A target view is required.");
        }
        return nil;
    }
    if (![operations isKindOfClass:NSArray.class]) {
        if (error) {
            *error =
                AVX512Error(@"Operations must be an NSArray.");
        }
        return nil;
    }
    NSString *sessionID =
    ...
    if (!documentsPath) {
        if (error) {
            *error =
                AVX512Error(
                    @"Could not locate the Documents directory.");
        }
        return nil;
    }
    NSURL *root =
    ...
    NSDictionary *receipt =
        AVX512GenerationReceipt(
            sessionID,
            targetClassName,
            safeOperations);
    NSError *writeError = nil;
    if (!AVX512WriteString(
            header,
            headerURL,
            &writeError)) {
        if (error) {
